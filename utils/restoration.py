...
from utils import import_function, load_project_config, join_path


def restore_params(run_id: str, run_pattern: str = 'GEAR-{}') -> Dict:
    """Restore params from run id"""
    project_config = load_project_config()
    run_dir = join_path(project_config['runs_directory'], run_pattern.format(run_id))
...
def get_transforms(data_flow_spec: Dict,
                   skip_resizing: bool = False) -> Tuple[Callable[[Any], torch.Tensor], Callable[[Any], torch.Tensor]]:
    ...
    trn = torchvision.transforms.Compose(
        [
            torchvision.transforms.Resize(img_size) if not skip_resizing else lambda x: x,
            torchvision.transforms.Grayscale(num_output_channels=len(norm_stats['mean']))
...
def choose_best_weights(artifacts_dir: str, use_metrics: Dict[str, Callable]) -> Dict[str, str]:
    ...
    for metric_name, best_fn in use_metrics.items():
        matched_files = [s for s in files if metric_name in s and s.endswith('.pth')]
        filenames_with_values = {s: float(s.split('=')[-1].replace('.pth', '')) for s in matched_files}
        if len(filenames_with_values) == 0:
            logging.warning(f'Can not find any weights for metric: {metric_name}')
            continue
        filename, _ = best_fn(filenames_with_values.items(), key=lambda v: v[1])
