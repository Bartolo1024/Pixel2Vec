...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, anchor_embedding: torch.Tensor,
        ...
        positive_product = anchor_embedding.bmm(positive_embedding).flatten(
            ['N', 'X', 'Y'], 'N')
        negative_product = anchor_embedding.bmm(negative_embedding).flatten(
            ['N', 'X', 'Y'], 'N')
        return (-self.log_sigmoid(positive_product) -
                self.log_sigmoid(-negative_product)).mean()
