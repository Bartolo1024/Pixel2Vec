...
class BaseEncoderBlock(nn.Sequential):
    ...
    def __init__(self,
        ...
        block = [
            nm.Conv2d(in_features,
                      out_features,
                      kernel_size=kernel_size,
                      dilation=dilation,
                      padding=padding),
            nm.BatchNorm2d(out_features)
...
class UNetResBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.adapter = nm.Conv2d(
            in_features, out_features,
            kernel_size=1) if in_features != out_features else None
...
class SimpleUNet(nn.Module):
    def __init__(
        ...
        self.encoder_blocks = nn.ModuleList([
            encoder_block_fn(in_features=features * 2**idx,
                             out_features=2 * features * 2**idx,
        ...
        self.pad = nm.ReflectionPad2d(
            sum([
                2 if getattr(b, 'skip_padding', False) else 0
                for b in self.encoder_blocks
            ]) + 2)
        self.pool = getattr(nm,
                            pool_fn)(scale) if scale > 1 else nn.Sequential()

        self.decoder_blocks = nn.ModuleList([
            decoder_block_fn(in_features=features * 2**idx *
                             (2 if idx == num_blocks - 1 else 4),
